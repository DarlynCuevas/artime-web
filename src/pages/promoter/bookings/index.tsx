    ...
    <div className="bg-white border border-slate-200 rounded-3xl p-16 text-center space-y-4">
      ...
      <p className="text-slate-500 text-sm max-w-xs mx-auto">
        {filtered
          ...
          : 'Los pedidos de contratación aparecerán aquí.'}
      </p>
    </div>
  );
}
...
function PromoterBookingsPage() {
  ...
  const filteredBookings = useMemo(() => {
    ...
    let base = (!tab || tab.statuses.length === 0)
      ? bookings
      : bookings.filter((b) => (tab.statuses as readonly string[]).includes(b.status));

    // Lógica especial de promotor: en CONFIRMED no incluir PENDING
    if (activeTab === 'CONFIRMED') {
      base = base.filter((b) => b.status !== 'PENDING');
    }

    // Sort: PENDING primero ordenados por fecha de creación desc
          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-10">
            <div className="space-y-1.5">
              <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-white/5 border border-white/10 text-amber-400 text-xs font-bold uppercase tracking-wider mb-1">
                <Music2 className="w-3.5 h-3.5" /> Promotor
              </div>
              ...
              <p className="text-white/50 text-base">
                Gestiona tus contrataciones de artistas y eventos.
              </p>
            </div>
